...
public class CheckerTests {
  ...
  public void givenTooShortInputNameWhenValidateThenFails() {
    ...
    assertThrows(InputException.InvalidParam.class, () -> isCorrectInputName(inputName), "Invalid parameter");
  }

  @ParameterizedTest
  @NullSource
  public void givenNullInputWhenValidateThenFails(String input) {
    assertThrows(InputException.NoData.class, () -> isCorrectInputName(input), "Empty input is not allowed");
  }

  @Test
  public void givenTooLongFileNameWhenValidateThenFails() {
    String fileName = generateRandomStringOfLength(500);
    assertThrows(InputException.InvalidParam.class, () -> isCorrectFileName(fileName), "Invalid parameter");
  }

  @Test
  public void givenUnexistingFileWhenValidateThenFails() {
    String fileName = generateRandomStringOfLength(5);
    assertThrows(InputException.NoFile.class, () -> isFileExisted(fileName), String.format("Given %s is not found", fileName));
  }

  @ParameterizedTest
  @NullSource
  public void givenNoInputFilesWhenValidateThenFails(String[] inputFiles) {
    assertThrows(InputException.NoData.class, () -> isCorrectInputFiles(inputFiles), "Empty input is not allowed");
  }
}
